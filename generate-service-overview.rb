...
raise ArgumentError, "Pager Duty team must be present if memory usage opt in monitor is in use  (define with -p)" if (options[:pagerduty_team].nil? || options[:pagerduty_team].empty?) && options[:mem_usage_opt_in] == true
